# 🪟 Guide Windows - SFEIR School DBT (Mode Portable Formateur)

Guide simplifié pour les formateurs **Windows sans droits administrateur**.
...
```batch
cd C:\chemin\vers\sfeir-school-dbt\tools\formateur\portable\scripts
```
...
Toutes les commandes se lancent depuis le dossier `tools\formateur\portable` :

```batch
cd C:\chemin\vers\sfeir-school-dbt\tools\formateur\portable
```
...
dbt run --profile sfeir_trainer_portable
dbt test --profile sfeir_trainer_portable
dbt run --select my_model --profile sfeir_trainer_portable
```
...
venv\Scripts\dbt.exe run --profile sfeir_trainer_portable
venv\Scripts\dbt.exe test --profile sfeir_trainer_portable
```
...
function dbt-portable {
    & "C:\chemin\vers\sfeir-school-dbt\tools\formateur\portable\venv\Scripts\dbt.exe" @args --profile sfeir_trainer_portable
}
...
### Distribuer le projet aux étudiants

1. **Option 1 : Clé USB**
   - Copiez tout le dossier `sfeir-school-dbt` sur une clé
   - Les étudiants copient sur leur PC
   - Chacun lance `bootstrap.bat` dans son dossier

2. **Option 2 : Git**
   - Les étudiants clonent le repository
   - Chacun lance `bootstrap.bat`

3. **Option 3 : Archive ZIP**
   - Créez un ZIP du projet
   - Partagez via drive/email
   - Les étudiants extraient et lancent `bootstrap.bat`

### Vérifier que tout fonctionne

Demandez à tous les étudiants de lancer :
```batch
dbt-debug.bat
```

Si tous voient `All checks passed!`, c'est bon ! ✅
