...
echo -e "${GREEN}╔═══════════════════════════════════════════════════╗${NC}"
echo -e "${GREEN}║  SFEIR School DBT - Setup Étudiant Portable      ║${NC}"
echo -e "${GREEN}╚═══════════════════════════════════════════════════╝${NC}"
echo ""
...
if [ ! -f ~/.dbt/profiles.yml ]; then
    ...
    if ! grep -q "sfeir_student_portable" ~/.dbt/profiles.yml; then
        ...
        echo -e "${GREEN}✓ Profil sfeir_student_portable ajouté à ~/.dbt/profiles.yml${NC}"
    else
        echo -e "${GREEN}✓ Profil sfeir_student_portable déjà présent${NC}"
    fi
fi
...
../../../etudiant/portable/venv/bin/dbt seed --profiles-dir ~/.dbt --profile sfeir_student_portable
